defmodule TurboXml do
  ...
  def write_ele_attrs([{key, val} | rest]) do
    [
      ...
      normalize_name(key),
      "=\"",
      val |> to_string() |> string_escape(),
      "\"" | write_ele_attrs(rest)
    ]
  end
  ...
  def validate_tag(<<l::utf8, _rest::binary>>)
      when not (l in [?_, ?-, ?., ?:]) and not (l in ?a..?z) and not (l in ?A..?Z) and
             not (l in ?0..?9) do
